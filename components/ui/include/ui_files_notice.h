 ...
 * are the menu entries that can fail for a reason the user can fix, and the
 * screen switches optimistically before the controller accepts the command.
 * Without a message the user just lands on an empty browser and has no idea
 * whether the medium is missing, unreadable, or simply empty.
 *
 * `source` only picks the wording - a card is not a flash drive and telling
 * someone to insert the wrong one sends them to the wrong slot.
 *
...
// True while the medium is mounted and readable, whatever is on it.
//
// Deliberately not ui_files_can_open(): an empty directory still has a ".."
