...
void ui_feed_model_init(ui_feed_model_t *model, uint8_t initial_index)
{
    ...
    model->visible = UI_MENU_VISIBLE_ALL;
}

void ui_feed_model_set_source_visible(ui_feed_model_t *model, ui_menu_item_t item,
                                      bool visible)
{
    if (model == NULL || item >= UI_MENU_ITEM_COUNT) return;
    if (visible) {
        model->visible |= UI_MENU_VISIBLE(item);
    } else {
        model->visible &= ~UI_MENU_VISIBLE(item);
    }
    if (!ui_menu_item_is_visible((ui_menu_item_t)model->selected, model->visible)) {
        model->selected = UI_FEED_INTERNET_RADIO;
    }
}

ui_menu_visible_mask_t ui_feed_model_visible_mask(const ui_feed_model_t *model)
{
    return model == NULL ? UI_MENU_VISIBLE_ALL : model->visible;
}
...
bool ui_feed_model_select_source(ui_feed_model_t *model, audio_source_t source)
{
    ...
    for (uint8_t index = 0U; index < UI_FEED_ITEM_COUNT; ++index) {
        if (!ui_menu_item_is_visible((ui_menu_item_t)index, model->visible)) continue;
