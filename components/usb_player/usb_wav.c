...
usb_wav_result_t usb_wav_parse_header(const uint8_t *data, size_t length, usb_wav_info_t *info)
{
    if (data == NULL || info == NULL) return USB_WAV_INVALID;
    if (length < 12U) return USB_WAV_NEED_MORE_DATA;
    if (memcmp(data, "RIFF", 4U) != 0 || memcmp(data + 8U, "WAVE", 4U) != 0) {
        return USB_WAV_INVALID;
    }
    ...
    while (offset + 8U <= length) {
        ...
        if (chunk_size > (uint32_t)SIZE_MAX - body) return USB_WAV_INVALID;

        if (memcmp(id, "fmt ", 4U) == 0) {
            if (chunk_size < 16U) return USB_WAV_INVALID;
            if (body + 16U > length) return USB_WAV_NEED_MORE_DATA;
            ...
            if (format_tag == WAV_FORMAT_EXTENSIBLE) {
                ...
                if (chunk_size < 26U) return USB_WAV_INVALID;
                if (body + 26U > length) return USB_WAV_NEED_MORE_DATA;
                format_tag = read_u16(data + body + 24U);
            }
            if (format_tag != WAV_FORMAT_PCM) return USB_WAV_UNSUPPORTED;
            ...
            if (info->bits_per_sample != 16U) return USB_WAV_UNSUPPORTED;
            if (info->channels != 1U && info->channels != 2U) return USB_WAV_UNSUPPORTED;
            if (info->sample_rate == 0U) return USB_WAV_INVALID;
            have_format = true;
            ...
            if (!have_format) return USB_WAV_INVALID;
        ...
        if (advance > SIZE_MAX - body) return USB_WAV_INVALID;
        offset = body + advance;
    }
    return USB_WAV_NEED_MORE_DATA;
}
