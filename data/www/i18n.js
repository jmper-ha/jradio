...
(() => {
  ...
  const DICT = {
    ...
    'hw.lead': ['Что подключено к модулю и на какие выводы. Результат — файл board.csv.',
                'What is wired to the module, and to which pins. The result is a board.csv file.'],
    'hw.parts': ['Что подключено', 'What is wired'],
    'hw.module': ['Модуль ESP32-S3-WROOM-1', 'The ESP32-S3-WROOM-1 module'],
    ...
    'hw.downloaded': ['board.csv скачан', 'board.csv downloaded'],
    'hw.import': ['Вставить свой board.csv', 'Paste your own board.csv'],
    'hw.import_placeholder': ['# key,value', '# key,value'],
    ...
    'hw.dev.tft': ['Экран', 'Display'],
    'hw.dev.encoder': ['Ручка', 'Knob'],
    ...
    'hw.f.module': ['Модуль', 'Module'],
    'hw.f.display': ['Панель', 'Panel'],
    ...
    'hw.f.i2c0_speed': ['Частота, Гц', 'Speed, Hz'],
    'hw.f.i2s0_bclk': ['BCLK', 'BCLK'], 'hw.f.i2s0_lrck': ['LRCK', 'LRCK'],
    'hw.f.i2s0_dout': ['DOUT (к ЦАП)', 'DOUT (to the DAC)'], 'hw.f.i2s0_din': ['DIN (от тюнера)', 'DIN (from the tuner)'],
    'hw.f.i2s0_mclk': ['MCLK', 'MCLK'],
    'hw.f.uart1_tx': ['TX', 'TX'], 'hw.f.uart1_rx': ['RX', 'RX'],
    'hw.f.tft_spi': ['Шина', 'Bus'], 'hw.f.tft_cs': ['CS', 'CS'], 'hw.f.tft_dc': ['DC', 'DC'],
    ...
    'hw.f.button_sleep': ['F1 — сон', 'F1 - sleep'], 'hw.f.button_quick_menu': ['F2 — быстрые настройки', 'F2 - quick panel'],
    'hw.f.button_prev': ['F3 — назад', 'F3 - previous'], 'hw.f.button_next': ['F4 — вперёд', 'F4 - next'],
    ...
    'hw.f.usb_vbus_switched': ['VBUS через ключ', 'VBUS through a switch'],
    'hw.f.sd_spi': ['Шина', 'Bus'], 'hw.f.sd_cs': ['CS', 'CS'], 'hw.f.sd_card_detect': ['Датчик карты', 'Card detect'],
    'hw.f.bluetooth': ['Модуль', 'Module'], 'hw.f.bt_uart': ['Шина', 'Bus'],
    'hw.f.fm_tuner': ['Микросхема', 'Chip'], 'hw.f.fm_i2c': ['Управление', 'Control'], 'hw.f.fm_i2s': ['Звук', 'Audio'],
    'hw.f.rtc': ['Микросхема', 'Chip'], 'hw.f.rtc_i2c': ['Шина', 'Bus'], 'hw.f.rtc_int': ['INT (будильник)', 'INT (alarm)'],
    ...
    'hw.s.i2s0_dout': ['I2S DOUT', 'I2S DOUT'], 'hw.s.i2s0_din': ['I2S DIN', 'I2S DIN'], 'hw.s.i2s0_mclk': ['I2S MCLK', 'I2S MCLK'],
    ...
    'hw.s.encoder_right': ['Ручка A', 'Knob A'], 'hw.s.encoder_left': ['Ручка B', 'Knob B'], 'hw.s.encoder_button': ['Ручка ⏎', 'Knob ⏎'],
    'hw.s.button_sleep': ['F1', 'F1'], 'hw.s.button_quick_menu': ['F2', 'F2'], 'hw.s.button_prev': ['F3', 'F3'], 'hw.s.button_next': ['F4', 'F4'],
    ...
    'hw.s.sd_cs': ['SD CS', 'SD CS'], 'hw.s.sd_card_detect': ['SD CD', 'SD CD'], 'hw.s.rtc_int': ['RTC INT', 'RTC INT'],
    ...
    'hw.opt.ds3231': ['DS3231', 'DS3231'], 'hw.opt.pcf8563': ['PCF8563', 'PCF8563'],
    'hw.opt.esp32s3_n16r8': ['N16R8 — octal PSRAM', 'N16R8 - octal PSRAM'],
    'hw.opt.esp32s3_n8r2': ['N8R2 — quad PSRAM', 'N8R2 - quad PSRAM'],
    ...
    'hw.opt.0': ['I2S0', 'I2S0'], 'hw.opt.1': ['UART1', 'UART1'], 'hw.opt.2': ['SPI2', 'SPI2'], 'hw.opt.3': ['SPI3', 'SPI3'],
    ...
    'hw.err.sleep_not_rtc': ['F1 на GPIO {gpio}: будить чип умеют только выводы 0–21, сон предлагаться не будет',
                             'F1 on GPIO {gpio}: only pins 0-21 can wake the chip, so sleep will not be offered'],
    'hw.err.rtc_int_not_rtc': ['INT часов на GPIO {gpio}: разбудить из сна сможет только вывод 0–21',
                               'The clock\'s INT on GPIO {gpio}: only a pin 0-21 can wake the chip'],
    ...
    'note.backup': ['В архив попадает всё, что устройство знает о себе: сети Wi-Fi, настройки, токен ЯМузыки и ключ погоды. Плейлист сохраняется отдельно, на странице плейлиста.',
                    'The archive holds everything the device knows about itself: Wi-Fi networks, settings, the Ya.Music token and the weather key. The playlist is saved separately, on the playlist page.'],
    'note.restore': ['Восстановить можно весь архив целиком или один файл из него — wifi.json, settings.csv, yandex.json или weather.json. После восстановления устройство перезагрузится.',
                     'Restore the whole archive or a single file out of it — wifi.json, settings.csv, yandex.json or weather.json. The device restarts afterwards.'],
