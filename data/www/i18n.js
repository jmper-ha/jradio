...
(() => {
  ...
  const DICT = {
    ...
    'note.screensaver': ['Заставка включается, когда к устройству никто не прикасается заданное число секунд: «Затемнение» опускает подсветку до яркости в покое, «Чёрный экран» гасит её, «Часы» гасят экран и плавно водят по нему время, дату, погоду и название того, что играет. Любое действие на корпусе будит экран; в двух тёмных режимах первое нажатие только будит.',
      'The screensaver starts once nobody has touched the device for the given number of seconds: "Dim" takes the backlight down to the idle brightness, "Blank" turns it off, "Clock" turns the screen off and floats the time, the date, the weather and what is playing across it. Any action on the device wakes the screen; in the two dark modes the first press only wakes it.'],
    'note.weather': ['Погода показывается рядом с часами. Координаты — широта и долгота в градусах, как их пишет карта: 55.75 и 37.62 для Москвы. Open-Meteo и wttr.in ключа не требуют; OpenWeatherMap требует — ключ хранится на устройстве отдельным файлом и входит в резервную копию; чтобы заменить ключ, введите новый, чтобы удалить — очистите поле.',
                     'The weather is shown beside the clock. The coordinates are latitude and longitude in degrees, as a map writes them: 55.75 and 37.62 for Moscow. Open-Meteo and wttr.in need no key; OpenWeatherMap does — the key is kept on the device in a file of its own and goes into the backup; type a new one to replace it, clear the field to remove it.'],
