...
static void test_elapsed_counts_what_was_played_not_what_was_read(void)
{
    ...
    assert(file_track_elapsed_seconds(176400U, 44100U, 2U, 16U) == 1U);
    assert(file_track_elapsed_seconds(176400U * 161U, 44100U, 2U, 16U) == 161U);
    /* Part of a second rounds down - a clock that reads ahead of the audio is
     * worse than one a fraction behind. */
    assert(file_track_elapsed_seconds(176399U, 44100U, 2U, 16U) == 0U);
}
...
static void test_elapsed_follows_the_format_not_a_fixed_rate(void)
{
    ...
    assert(file_track_elapsed_seconds(44100U, 22050U, 1U, 16U) == 1U);
    /* 24-bit stereo at 48 kHz. */
    assert(file_track_elapsed_seconds(288000U, 48000U, 2U, 24U) == 1U);
}
...
static void test_elapsed_is_zero_until_the_format_is_known(void)
{
    ...
    assert(file_track_elapsed_seconds(1000000U, 0U, 2U, 16U) == 0U);
    assert(file_track_elapsed_seconds(1000000U, 44100U, 0U, 16U) == 0U);
    assert(file_track_elapsed_seconds(1000000U, 44100U, 2U, 0U) == 0U);
    assert(file_track_elapsed_seconds(0U, 44100U, 2U, 16U) == 0U);
}
...
static void test_total_comes_from_size_and_bitrate(void)
{
    ...
    assert(file_track_total_seconds(5120000U, 0U, 128U) == 320U);
    /* FLAC at 1058 kbps. */
    assert(file_track_total_seconds(52900000U, 0U, 1058U) == 400U);
}
...
static void test_the_header_is_not_counted_as_audio(void)
{
    ...
    assert(file_track_total_seconds(size, 44U, 1411U) == file_track_total_seconds(176400U, 0U, 1411U));
    /* A file that is nothing but header has no duration. */
    assert(file_track_total_seconds(44U, 44U, 1411U) == 0U);
    assert(file_track_total_seconds(10U, 44U, 1411U) == 0U);
}
...
static void test_a_flac_gets_its_length_from_its_sample_count(void)
{
    ...
    assert(file_track_sampled_seconds(22050000U, 44100U) == 500U);
    assert(file_track_sampled_seconds(21987648U, 44100U) == 498U);
    // 96 kHz is the same arithmetic and must not be assumed away.
    assert(file_track_sampled_seconds(9600000U, 96000U) == 100U);
}
...
static void test_a_stream_written_to_a_file_has_no_length(void)
{
    ...
    assert(file_track_sampled_seconds(0U, 44100U) == 0U);
    assert(file_track_sampled_seconds(22050000U, 0U) == 0U);
}
...
static void test_a_format_with_no_stated_rate_gets_an_average(void)
{
    ...
    assert(file_track_average_bitrate_kbps(62500000U, 0U, 500U) == 1000U);
    // The header is not audio, here as everywhere else.
    assert(file_track_average_bitrate_kbps(62500000U + 8000U, 8000U, 500U) == 1000U);
    // And it has to round-trip with the seek: an average is what turns
    // seconds back into an offset.
    assert(file_track_seek_offset(62500000U, 0U, 1000U, 250U) == 31250000U);
}

static void test_an_average_of_nothing_is_not_reported(void)
{
    assert(file_track_average_bitrate_kbps(62500000U, 0U, 0U) == 0U);
    assert(file_track_average_bitrate_kbps(100U, 200U, 10U) == 0U);
    // Clamped to the field rather than wrapped inside it.
    assert(file_track_average_bitrate_kbps(UINT64_C(1) << 40, 0U, 1U) == UINT16_MAX);
}
...
static void test_an_unknown_bitrate_gives_no_duration(void)
{
    ...
    assert(file_track_total_seconds(5120000U, 0U, 0U) == 0U);
}

static void test_the_bar_is_hidden_when_there_is_nothing_to_show(void)
{
    uint8_t percent = 42U;
    assert(!file_track_progress_percent(10U, 0U, &percent));
    assert(!file_track_progress_percent(10U, 100U, NULL));
}

static void test_the_bar_tracks_the_position(void)
{
    uint8_t percent = 0U;
    assert(file_track_progress_percent(0U, 400U, &percent));
    assert(percent == 0U);
    assert(file_track_progress_percent(100U, 400U, &percent));
    assert(percent == 25U);
    assert(file_track_progress_percent(400U, 400U, &percent));
    assert(percent == 100U);
}
...
static void test_the_bar_cannot_overshoot_its_track(void)
{
    ...
    uint8_t percent = 0U;
    assert(file_track_progress_percent(900U, 400U, &percent));
    assert(percent == 100U);
}
...
{
    char text[16];
    file_track_time_text(text, sizeof(text), 161U);
    assert(strcmp(text, "2:41") == 0);
    /* Seconds always padded, minutes not - "2:05", never "2:5" or "02:05". */
    file_track_time_text(text, sizeof(text), 125U);
    assert(strcmp(text, "2:05") == 0);
    file_track_time_text(text, sizeof(text), 0U);
    assert(strcmp(text, "0:00") == 0);
    file_track_time_text(text, sizeof(text), 59U);
...
static void test_a_long_track_grows_an_hours_field(void)
{
    ...
    char text[16];
    file_track_time_text(text, sizeof(text), 3723U);
    assert(strcmp(text, "1:02:03") == 0);
    file_track_time_text(text, sizeof(text), 3600U);
    assert(strcmp(text, "1:00:00") == 0);
    /* Just short of an hour keeps the short form. */
    file_track_time_text(text, sizeof(text), 3599U);
    assert(strcmp(text, "59:59") == 0);

    file_track_time_text(NULL, sizeof(text), 10U);
    file_track_time_text(text, 0U, 10U);
}
...
static void test_a_seek_lands_where_the_length_estimate_says_it_should(void)
{
    ...
    assert(file_track_seek_offset(4000000U, 0U, 128U, 60U) == 960000U);
    /* The header is not audio, so it is added on rather than seeked into. */
    assert(file_track_seek_offset(4000000U, 2048U, 128U, 60U) == 960000U + 2048U);
    /* The start of the track is the start of the audio, whatever precedes it. */
    assert(file_track_seek_offset(4000000U, 2048U, 128U, 0U) == 2048U);
}
...
static void test_a_seek_never_leaves_the_file(void)
{
    ...
    assert(file_track_seek_offset(500000U, 0U, 128U, 600U) == 500000U);
    ...
    assert(file_track_seek_offset(4000000U, 2048U, 0U, 60U) == 2048U);
    /* A file smaller than its own header is corrupt; do not seek into it. */
    assert(file_track_seek_offset(1024U, 2048U, 128U, 60U) == 2048U);
}
...
static void test_the_position_carries_on_from_where_the_seek_landed(void)
{
    ...
    const uint64_t bytes = file_track_pcm_bytes(90U, 44100U, 2U, 16U);
    assert(bytes == 90ULL * 44100U * 4U);
    assert(file_track_elapsed_seconds(bytes, 44100U, 2U, 16U) == 90U);

    /* Nothing known about the format yet means no byte count to aim for. */
    assert(file_track_pcm_bytes(90U, 0U, 2U, 16U) == 0U);
    assert(file_track_pcm_bytes(90U, 44100U, 0U, 16U) == 0U);
...
int main(void)
{
    ...
    puts("file_track_progress tests passed");
    return 0;
}
