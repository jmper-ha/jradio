...
static void test_autoplay_off_always_opens_the_home_screen(void)
{
    ...
    assert(ui_autoplay_decide(&radio, FILE_BROWSER_MEDIA_READY, true) == UI_AUTOPLAY_HOME);

    const device_settings_t usb = settings(false, DEVICE_LAST_SOURCE_USB, "/usb0/a.mp3");
    assert(ui_autoplay_decide(&usb, FILE_BROWSER_MEDIA_READY, true) == UI_AUTOPLAY_HOME);
}
...
static void test_nothing_played_before_opens_the_home_screen(void)
{
    ...
    assert(ui_autoplay_decide(&value, FILE_BROWSER_MEDIA_READY, true) == UI_AUTOPLAY_HOME);
    assert(ui_autoplay_decide(&value, FILE_BROWSER_MEDIA_ABSENT, false) == UI_AUTOPLAY_HOME);
}
...
static void test_radio_resumes_whatever_the_drive_is_doing(void)
{
    ...
    assert(ui_autoplay_decide(&value, FILE_BROWSER_MEDIA_READY, true) == UI_AUTOPLAY_RADIO);
    assert(ui_autoplay_decide(&value, FILE_BROWSER_MEDIA_ABSENT, false) == UI_AUTOPLAY_RADIO);
    assert(ui_autoplay_decide(&value, FILE_BROWSER_MEDIA_UNREADABLE, false) == UI_AUTOPLAY_RADIO);
}
...
static void test_usb_resumes_the_same_file_when_it_is_still_there(void)
{
    ...
    assert(ui_autoplay_decide(&value, FILE_BROWSER_MEDIA_READY, true) == UI_AUTOPLAY_FILE);
}
...
static void test_a_missing_file_falls_back_to_the_browser(void)
{
    ...
    assert(ui_autoplay_decide(&value, FILE_BROWSER_MEDIA_READY, false) ==
...
static void test_usb_last_but_nothing_played_opens_the_browser(void)
{
    ...
    assert(ui_autoplay_decide(&value, FILE_BROWSER_MEDIA_READY, true) ==
           UI_AUTOPLAY_FILE_BROWSER);
}
...
static void test_no_drive_says_so_rather_than_opening_an_empty_browser(void)
{
    ...
    assert(ui_autoplay_decide(&value, FILE_BROWSER_MEDIA_ABSENT, false) ==
    ...
    assert(ui_autoplay_decide(&value, FILE_BROWSER_MEDIA_UNREADABLE, true) ==
           UI_AUTOPLAY_FILE_UNAVAILABLE);
}
...
static void test_the_drive_check_comes_before_the_file_check(void)
{
    ...
    const device_settings_t value = settings(true, DEVICE_LAST_SOURCE_USB, "/usb0/a.mp3");
    assert(ui_autoplay_decide(&value, FILE_BROWSER_MEDIA_ABSENT, true) ==
           UI_AUTOPLAY_FILE_UNAVAILABLE);
}

static void test_a_missing_settings_struct_opens_the_home_screen(void)
{
    assert(ui_autoplay_decide(NULL, FILE_BROWSER_MEDIA_READY, true) == UI_AUTOPLAY_HOME);
}
