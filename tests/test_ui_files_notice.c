...
{
    assert(ui_files_notice(FILE_BROWSER_MEDIA_READY, 8U) == NULL);
    assert(ui_files_can_open(FILE_BROWSER_MEDIA_READY, 8U));
    assert(ui_files_can_open(FILE_BROWSER_MEDIA_READY, 1U));
}

static void test_a_missing_drive_asks_for_one(void)
{
    const char *notice = ui_files_notice(FILE_BROWSER_MEDIA_ABSENT, 0U);
    assert(notice != NULL);
    assert(strcmp(notice, "Вставьте USB-флешку") == 0);
    assert(!ui_files_can_open(FILE_BROWSER_MEDIA_ABSENT, 0U));
}
...
static void test_an_unreadable_drive_is_not_told_to_be_inserted(void)
{
     ...
     * place. */
    const char *notice = ui_files_notice(FILE_BROWSER_MEDIA_UNREADABLE, 0U);
    assert(notice != NULL);
    assert(strstr(notice, "Вставьте") == NULL);
    assert(strstr(notice, "FAT32") != NULL);
    assert(!ui_files_can_open(FILE_BROWSER_MEDIA_UNREADABLE, 0U));
}
...
static void test_an_unreadable_drive_stays_unreadable_whatever_the_count(void)
{
     ...
     * usable. */
    const char *notice = ui_files_notice(FILE_BROWSER_MEDIA_UNREADABLE, 12U);
    assert(notice != NULL);
    assert(!ui_files_can_open(FILE_BROWSER_MEDIA_UNREADABLE, 12U));
}

static void test_an_empty_drive_says_so_rather_than_showing_a_blank_list(void)
{
    const char *notice = ui_files_notice(FILE_BROWSER_MEDIA_READY, 0U);
    assert(notice != NULL);
    assert(strcmp(notice, "На флешке нет файлов") == 0);
    /* Not an error, but an empty browser looks like one, so it is still not
     * worth opening. */
    assert(!ui_files_can_open(FILE_BROWSER_MEDIA_READY, 0U));
}

static void test_each_state_gets_its_own_wording(void)
{
    const char *absent = ui_files_notice(FILE_BROWSER_MEDIA_ABSENT, 0U);
    const char *unreadable = ui_files_notice(FILE_BROWSER_MEDIA_UNREADABLE, 0U);
    const char *empty = ui_files_notice(FILE_BROWSER_MEDIA_READY, 0U);
...
static void test_media_presence_is_not_the_same_question_as_openability(void)
{
    ...
    assert(ui_files_media_present(FILE_BROWSER_MEDIA_READY) &&
           !ui_files_can_open(FILE_BROWSER_MEDIA_READY, 0U));
