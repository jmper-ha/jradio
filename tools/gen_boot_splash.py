#!/usr/bin/env python3
"""Draw the 320x240 boot splash: a green magic-eye tube and the jRadio wordmark.

Everything is drawn at SS times the final size and boxed down, which is what
keeps the wedge edges and the rotated lettering smooth at 320x240 - the panel
is RGB565 and has no antialiasing of its own.

Emits the preview PNG and the run-length encoded pixels the firmware draws
...
def main():
    ...
    image = compose(CHOSEN, path)
    print("wrote", path)

    source = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                          "components", "board", "boot_splash_image.c")
    pairs = write_c_source(image, source)
    print(f"wrote {source} ({len(pairs)} runs, {len(pairs) * 4} bytes of flash)")
