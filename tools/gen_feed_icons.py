...
# Icons that are not carousel slots and need only their own size. The note is
...
for _name in WEATHER_ICONS:
    EXTRA_SIZES[_name] = WEATHER_SIZES
...
# The strike through the dislike heart, in grid units. The gap is knocked out
# of the glyph before the bar is drawn over it: without it, at 16 px the bar and
# the outline merge into one blob and the mark reads as a smudge rather than as
# a crossing-out. Both numbers are Material's own proportions for its *_off
# icons, which is what makes this heart sit beside the other two.
SLASH_GAP = 3.4
SLASH_STROKE = 1.9
SLASH_FROM = (4.2, 4.2)
SLASH_TO = (19.8, 19.8)
...
def draw_glyph(size, font_path, codepoint, filled=False, slash=False):
    image, d, s = canvas(size)
    ...
    if slash:
        # Drawn on the supersampled canvas, before the downsample, or the bar
        # would be a hard-edged one pixel wide while the glyph around it is
        # smooth. The gap pass comes first and erases; the stroke pass fills.
        for width, fill in ((SLASH_GAP, 0), (SLASH_STROKE, 255)):
            d.line([SLASH_FROM[0] * s, SLASH_FROM[1] * s, SLASH_TO[0] * s, SLASH_TO[1] * s],
                   fill=fill, width=max(1, round(width * s)))
            # Round caps: PIL's line has none, and a square end on a diagonal
            # bar is visible even at this size.
            for x, y in (SLASH_FROM, SLASH_TO):
                r = width * s / 2.0
...
ICONS = (
    ...
    # The third state of the same mark: the account has rejected this track.
    # The same outline heart struck through, rather than a thumb-down, so the
    # three states of one mark stay one shape.
    ("heart_slash", ("glyph_slash", 0xE87D)),
    ...
    ("partly_cloudy_night", ("glyph", 0xF174)),
    ("cloudy", ("glyph", 0xF15C)),
    ("fog", ("glyph", 0xE818)),
    ("rain", ("glyph", 0xF176)),
    ("snow", ("glyph", 0xE2CD)),
    ("sleet", ("glyph", 0xF60B)),
    ("thunderstorm", ("glyph", 0xEBDB)),
)
